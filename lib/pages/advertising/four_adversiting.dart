...
import 'five_adversiting.dart';

int clicado = -1;

class FourAdversiting extends StatelessWidget {
  const FourAdversiting({super.key});
          ...
          children: [
            Align(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  Navigator.pop(context); //volta ou fecha
                  // Ação do botão
                },
                iconSize: 20,
              ),
            ...
            // Imagem
            Image.asset(
              'imgs/imgs_adversiting/morango_adversiting.png',
              width: 250,
              height: 250,
              fit: BoxFit.contain,
            ),
            ...
            ElevatedButton(
              onPressed: () {
                if (clicado != -1) {
                  //Navegar para a quinta tela com animação personalizada
                  Navigator.push(
                    context,
                    PageRouteBuilder(
                      pageBuilder: (context, animation, secondaryAnimation) =>
                          const FiveAdversiting(),
                      transitionsBuilder:
...
class _SelectableButtonsState extends State<SelectableButtons> {
  ...
  Widget build(BuildContext context) {
    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        _buildButton(0, 'Não, minha alimentação não é boa'),
        const SizedBox(height: 16),
        _buildButton(1, 'Mais ou menos, tenho trabalhado para melhorar'),
        const SizedBox(height: 16),
        _buildButton(2, 'Sim, minha alimentação é boa'),
        const SizedBox(height: 16),
        _buildButton(3, 'Sim, minha alimentação é muito boa.'),
      ],
    );
  }
