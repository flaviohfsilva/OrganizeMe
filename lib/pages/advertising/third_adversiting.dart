...
import 'four_adversiting.dart';

int clicado = -1;

class ThirdAdversiting extends StatelessWidget {
  const ThirdAdversiting({super.key});
        ...
        child: Column(
          ...
          children: [
            Align(
              ...
              child: IconButton(
                icon: Image.asset('imgs/imgs_adversiting/btn_voltar.png'),
                onPressed: () {
            ...
            const SizedBox(
              child: Text(
                'Você costuma procrastinar as atividades do dia a dia?',
            ...
            // Imagem
            Image.asset(
              'imgs/imgs_adversiting/emoji_pensando.png',
              width: 250,
              height: 250,
              fit: BoxFit.contain,
            ),
            ...
            ElevatedButton(
              onPressed: () {
                if (clicado != -1) {
                  // Navegar para a quarta tela com animação personalizada
                  Navigator.push(
                    context,
                    PageRouteBuilder(
                      pageBuilder: (context, animation, secondaryAnimation) =>
                          const FourAdversiting(),
                      transitionsBuilder:
...
class _SelectableButtonsState extends State<SelectableButtons> {
  ...
  Widget build(BuildContext context) {
    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        _buildButton(0, 'Sim, tenho dificuldade para terminar as atividades.'),
        const SizedBox(height: 16),
        _buildButton(
            1, 'Ás vezes sinto dificuldade para seguir minha programação.'),
        const SizedBox(height: 16),
        _buildButton(2, 'Não, é raro não seguir comminha programação'),
        const SizedBox(height: 16),
        _buildButton(3, 'Não, não sinto nenhuma dificuldade'),
      ],
    );
  }
